...
{
  cache->array = cache->_init( &cache->array_length, &cache->max_length );
  if ( cache->array == NULL )
  {
    cache->data = NULL;
    cache->length = 1;
...
void cache_destroy(Cache *cache)
{
  ...
  if ( cache->array != NULL ) g_free( cache->array );
  if ( cache->data != NULL ) g_free( cache->data );
  cache->array_length = 0;
  cache->length = 0;
}
